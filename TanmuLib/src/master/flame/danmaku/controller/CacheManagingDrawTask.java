...
public class CacheManagingDrawTask extends DrawTask {
    ...
    public class CacheManager {
        ...
        private boolean push(BaseDanmaku item, int itemSize) {
            ...
            mRealSize += size;
//Log.v("CACHE", "realsize:"+mRealSize + ",size" + size);
            return true;
        }
...
//Log.v(TAG,"dispatch_actions:"+mCacheTimer.currMillisecond+":"+mTimer.currMillisecond);
...
//Log.v("prepareCache", currScreenIndex+","+orderInScreen+"," + item.time+"skip:"+skip);
                    if (skip) {
                        continue;
                    }
...
//Log.i("cache","stop at :"+item.time+","+count+",size:"+danmakus.size()+","+message);
            ...
            private byte buildCache(BaseDanmaku item) {
                ...
                try {
                    ...
                    if (cache != null) {
                        danmaku.cache = null;
//Log.v("cache", danmaku.text+"DrawingCache hit!!:" + item.paintWidth + "," + danmaku.paintWidth);
...
//Log.v("cache", "break at push failed:" + mMaxSize);
                        }
                        return pushed ? RESULT_SUCCESS : RESULT_FAILED;
                    }

                } catch (OutOfMemoryError e) {
//Log.v("cache", "break at error: oom");
                    releaseDanmakuCache(item, cache);
                    return RESULT_FAILED;
                } catch (Exception e) {
//Log.v("cache", "break at exception:" + e.getMessage());
                    releaseDanmakuCache(item, cache);
                    return RESULT_FAILED;
                }
            }
