...
public class MypaihangActivity extends AgentActivity implements OnClickListener {
	...
	public void doShowNearby(final boolean isRefresh, boolean isLazyLoad) {
		doPullDate(isLazyLoad, "2037", new MCHttpCallBack() {
					...
					if (resultCode.equals("0")) {
						...
						if (isRefresh) {
							...
							if (itemsCount > 0) {
								...
								Toast.makeText(MypaihangActivity.this,
			...
			public void onError(MCHttpResp resp) {
				super.onError(resp);
				Toast.makeText(MypaihangActivity.this, resp.getErrorMessage(),
						Toast.LENGTH_SHORT).show();
				// 数据载入关闭提示;
				onFinishLoad();
			}
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				try {
					...
					if (EMGroupManager.getInstance().getGroup(
						...
						// 进入聊天页面
						Intent intent = new Intent(MypaihangActivity.this,
								ChatActivity.class);
						// it is group chat
						...
						intent.putExtra("roomId", new_list.get(position - 1)
								.getString("id"));
						MypaihangActivity.this.startActivity(intent);
					}
				} catch (Exception e) {
