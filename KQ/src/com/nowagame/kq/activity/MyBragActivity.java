...
public class MyBragActivity extends AgentActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		init();
		AddOnClickListener();
		adapter = new MyBragActivityAdapter(this);
	...
	public void doShowNearby(final boolean isRefresh, boolean isLazyLoad) {
		...
		doPullDate(isLazyLoad, action, new MCHttpCallBack() {
			...
			public void onSuccess(MCHttpResp resp) {
				...
				try {
					...
					if (resultCode.equals("0")) {
						...
						if (isRefresh) {
							records = record;
							adapter.setItem(records,action);
						} else {
							...
							if (itemsCount > 0) {
								...
								Toast.makeText(MyBragActivity.this, "没有更多数据了.",
						...
						Toast.makeText(MyBragActivity.this,
			...
			public void onError(MCHttpResp resp) {
				super.onError(resp);
				Toast.makeText(MyBragActivity.this, resp.getErrorMessage(),
						Toast.LENGTH_SHORT).show();
				// 数据载入关闭提示;
				onFinishLoad();
			}
	...
	class OnItemClickListener implements
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int option,
			...
			intent.setClass(MyBragActivity.this, BragDetailActivity.class);
