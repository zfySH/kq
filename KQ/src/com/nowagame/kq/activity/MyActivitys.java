...
public class MyActivitys extends AgentActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.abc_activity_activity);
	...
	public void doShowNearby(final boolean isRefresh, boolean isLazyLoad) {
		doPullDate(isLazyLoad, "2037", new MCHttpCallBack() {
					...
					if (resultCode.equals("0")) {
						...
						if (isRefresh) {
							...
							if (itemsCount > 0) {
								...
								Toast.makeText(MyActivitys.this, "没有更多数据了.",
						...
						Toast.makeText(MyActivitys.this,
			...
			public void onError(MCHttpResp resp) {
				super.onError(resp);
				Toast.makeText(MyActivitys.this, resp.getErrorMessage(),
						Toast.LENGTH_SHORT).show();
				// 数据载入关闭提示;
				onFinishLoad();
			}
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				try {
					...
					if (EMGroupManager.getInstance().getGroup(
						...
						Toast.makeText(MyActivitys.this, "您还不是该房间的成员，请联系房间管理员",
								Toast.LENGTH_SHORT).show();
					} else {
						// 进入聊天页面
						Intent intent = new Intent(MyActivitys.this,
								ChatActivity.class);
						// it is group chat
						...
						intent.putExtra("roomId", new_list.get(position - 1)
								.getString("id"));
						MyActivitys.this.startActivity(intent);
					}
				} catch (Exception e) {
