...
public abstract class HXSDKHelper {
    ...
    public synchronized boolean onInit(Context context){
        ...
        String hxModelAppName = hxModel.getAppProcessName();
        Log.d(TAG, "hxModelAppName : " + hxModelAppName);
        
        // 如果app启用了远程的service，此application:onCreate会被调用2次
        // 为了防止环信SDK被初始化2次，加此判断会保证SDK被初始化1次
        // 默认的app会在以包名为默认的process name下运行，如果查到的process name不是app的process name就立即返回
        if (processAppName == null || !processAppName.equalsIgnoreCase(hxModelAppName)) {
            Log.v(TAG, "enter the service process!");
