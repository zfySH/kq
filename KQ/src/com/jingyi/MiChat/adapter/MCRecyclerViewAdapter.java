...
import com.jingyi.MiChat.viewholder.ListViewLoadMoreViewHolder;
import com.jingyi.MiChat.widget.recyclerview.MCLoadingMoreView;

public abstract class MCRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
	...
	public MCRecyclerViewAdapter(Context context){
		this.ctx = context;
	...
	public final void notifyHeaderItemRangeInserted(int positionStart, int itemCount) {
		...
		if (positionStart < 0 || itemCount < 0 || positionStart + itemCount > newHeaderItemCount) {
			throw new IndexOutOfBoundsException("The given range [" + positionStart + " - " + (positionStart + itemCount - 1) + "] is not within the position bounds for header items [0 - "
	...
	public void notifyHeaderItemMoved(int fromPosition, int toPosition) {
		if (fromPosition < 0 || toPosition < 0 || fromPosition >= headerItemCount || toPosition >= headerItemCount) {
			throw new IndexOutOfBoundsException("The given fromPosition " + fromPosition + " or toPosition " + toPosition + " is not within the position bounds for header items [0 - "
					+ (headerItemCount - 1) + "].");
		}
	...
	public final void notifyContentItemRangeInserted(int positionStart, int itemCount) {
		...
		if (positionStart < 0 || itemCount < 0 || positionStart + itemCount > newContentItemCount) {
			throw new IndexOutOfBoundsException("The given range [" + positionStart + " - " + (positionStart + itemCount - 1) + "] is not within the position bounds for content items [0 - "
	...
	public final void notifyContentItemMoved(int fromPosition, int toPosition) {
		if (fromPosition < 0 || toPosition < 0 || fromPosition >= contentItemCount || toPosition >= contentItemCount) {
			throw new IndexOutOfBoundsException("The given fromPosition " + fromPosition + " or toPosition " + toPosition + " is not within the position bounds for content items [0 - "
					+ (contentItemCount - 1) + "].");
		}
	...
	public final void notifyFooterItemRangeInserted(int positionStart, int itemCount) {
		...
		if (positionStart < 0 || itemCount < 0 || positionStart + itemCount > newFooterItemCount) {
			throw new IndexOutOfBoundsException("The given range [" + positionStart + " - " + (positionStart + itemCount - 1) + "] is not within the position bounds for footer items [0 - "
	...
	public final void notifyFooterItemMoved(int fromPosition, int toPosition) {
		if (fromPosition < 0 || toPosition < 0 || fromPosition >= footerItemCount || toPosition >= footerItemCount) {
			throw new IndexOutOfBoundsException("The given fromPosition " + fromPosition + " or toPosition " + toPosition + " is not within the position bounds for footer items [0 - "
					+ (footerItemCount - 1) + "].");
		}
	 ...
	 * {@link #onCreateViewHolder(android.view.ViewGroup, int)}, but for header
	 * items.
	 *
	 * @param parent
	 *            the parent view.
	...
	/**
	 * This method works exactly the same as
	 * {@link #onCreateViewHolder(android.view.ViewGroup, int)}, but for footer
	 * items.
	 *
	 * @param parent
	 *            the parent view.
	...
	protected RecyclerView.ViewHolder onCreateFooterItemViewHolder(ViewGroup parent, int footerViewType) {
		if(loadComplete)
			return null;
		MCLoadingMoreView loadView = (MCLoadingMoreView) mInflater.inflate(R.layout.list_loading_more_view, parent, false);
		ListViewLoadMoreViewHolder holder = new ListViewLoadMoreViewHolder(loadView);
		return holder;
	}

	/**
	 * This method works exactly the same as
	 * {@link #onCreateViewHolder(android.view.ViewGroup, int)}, but for content
	 * items.
	 *
	 * @param parent
	 *            the parent view.
	...
	protected void onBindFooterItemViewHolder(RecyclerView.ViewHolder footerViewHolder, int position) {
		if(footerViewHolder instanceof ListViewLoadMoreViewHolder){
			ListViewLoadMoreViewHolder viewHolder = (ListViewLoadMoreViewHolder)footerViewHolder;
			if(showLoadMore){
				viewHolder.mLoadMoreView.setVisibility(View.VISIBLE);
			}else{
				viewHolder.mLoadMoreView.setVisibility(View.GONE);
			}
		}
